...
def load_data(selection='ALL'):
    ...
    # First, look for the data file in the same directory as this module (the project folder),
    # then fall back to the current working directory (so running from a different cwd still works).
    module_dir = os.path.dirname(os.path.abspath(__file__))
    primary_file_path = os.path.join(module_dir, 'dataset.xlsx')
    fallback_csv_path = os.path.join(module_dir, 'data.csv')
    ...
    if os.path.exists(primary_file_path):
        ...
        # Fall back to current working directory to support different run paths
        if os.path.exists('dataset.xlsx'):
            file_path = 'dataset.xlsx'
        elif os.path.exists('data.csv'):
            return load_data_from_csv('data.csv')
    ...
    try:
        ...
        optional_columns = ['P_H2_in', 'P_H2O_in']
        ...
        # Always ensure required columns exist for advanced models
        P_MeOH = df_clean['P_MeOH_in'].to_numpy(dtype=np.float64)
        P_O2   = df_clean['P_O2_in'].to_numpy(dtype=np.float64)

        # Handle H2 (missing in your dataset → assume 1)
        if 'P_H2_in' in df_clean.columns:
            P_H2 = df_clean['P_H2_in'].to_numpy(dtype=np.float64)
        else:
            print("⚠️ P_H2_in not found → assuming constant = 1")
            P_H2 = np.ones_like(P_MeOH)

        # Handle H2O (you already have this)
        if 'P_H2O_in' in df_clean.columns:
            P_H2O = df_clean['P_H2O_in'].to_numpy(dtype=np.float64)
        else:
            print("⚠️ P_H2O_in not found → assuming constant = 1")
            P_H2O = np.ones_like(P_MeOH)

        # Final x matrix (ALWAYS 4 columns now)
        xexp = np.column_stack((P_MeOH, P_O2, P_H2, P_H2O))
...
def load_data_from_csv(file_path):
    ...
    try:
        ...
        temp_exp = df_clean['Temp'].to_numpy(dtype=np.float64)
        pressure_columns = ['P_MeOH_in', 'P_O2_in'] + [col for col in optional_columns if col in df_clean.columns]
        xexp = df_clean[pressure_columns].to_numpy(dtype=np.float64)
        rate_exp = df_clean['R_HCHO'].to_numpy(dtype=np.float64)

        if len(pressure_columns) > 2:
            print(f"Loaded pressure columns: {pressure_columns}")
